...
struct ImGuiContext
{
    ...
    // zlogging
    bool                    zlogEnabled;
    FILE *                  zlogFile;                            // If != NULL zlog to stdout/ file
    ImGuiTextBuffer         zlogClipboard;                       // Accumulation buffer when zlog to clipboard. This is pointer so our GImGui static constructor doesn't call heap allocators.
    int                     zlogStartDepth;
    int                     zlogAutoExpandMaxDepth;
...
namespace ImGui
{
    ...
    IMGUI_API bool          IsClippedEx(const ImRect& bb, ImGuiID id, bool clip_even_when_zlogged);
    ...
    IMGUI_API bool          TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags = 0);                     // Consume previous SetNextTreeNodeOpened() data, if any. May return true when zlogging
