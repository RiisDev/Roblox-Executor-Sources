...
static void ShowExampleAppzlog(bool *p_open);
...
void ImGui::ShowDemoWindow(bool *p_open)
{
    ...
    static bool show_app_zlog = false;
    ...
    if (show_app_zlog)                 ShowExampleAppzlog(&show_app_zlog);
    ...
    if (ImGui::BeginMenuBar())
    {
        ...
        if (ImGui::BeginMenu("Examples"))
        {
            ...
            ImGui::MenuItem("zlog", NULL, &show_app_zlog);
    ...
    if (ImGui::CollapsingHeader("Window options"))
    {
        ...
        if (ImGui::TreeNode("Capture/zlogging"))
        {
            ImGui::TextWrapped("The zlogging API redirects all text output so you can easily capture the content of a window or a block. Tree nodes can be automatically expanded. You can also call ImGui::zlogText() to output directly to the zlog without a visual output.");
            ImGui::zlogButtons();
            ImGui::TreePop();
        }
    }
    ...
    if (ImGui::TreeNode("Colors"))
    {
        ...
        if (ImGui::Button("Export Unsaved"))
        {
            if (output_dest == 0)
                ...
                ImGui::zlogToTTY();
            ImGui::zlogText("ImVec4 *colors = ImGui::GetStyle().Colors;" IM_NEWLINE);
            for (int i = 0; i < ImGuiCol_COUNT; i++)
            {
                const ImVec4& col = style.Colors[i];
                const char *name = ImGui::GetStyleColorName(i);
                if (!output_only_modified || memcmp(&col, &ref->Colors[i], sizeof(ImVec4)) != 0)
                    ImGui::zlogText("colors[ImGuiCol_%s]%*s= ImVec4(%.2ff, %.2ff, %.2ff, %.2ff);" IM_NEWLINE, name, 23-(int)strlen(name), "", col.x, col.y, col.z, col.w);
            }
            ImGui::zlogFinish();
        }
...
struct ExampleAppConsole
{
    ...
    void    Addzlog(const char *fmt, ...) IM_FMTARGS(2)
    {
        // FIXME-OPT
        char buf[1024];
        va_list args;
    ...
    void    Draw(const char *title, bool *p_open)
    {
        ...
        if (ImGui::SmallButton("Add Dummy Text")) { Addzlog("%d some text", Items.Size); Addzlog("some more text"); Addzlog("display very important message here!"); } ImGui::SameLine();
        if (ImGui::SmallButton("Add Dummy Error")) { Addzlog("[error] something went wrong"); } ImGui::SameLine();
        if (ImGui::SmallButton("Clear")) { Clearzlog(); } ImGui::SameLine();
        bool copy_to_clipboard = ImGui::SmallButton("Copy"); ImGui::SameLine();
        if (ImGui::SmallButton("Scroll to bottom")) ScrollToBottom = true;
        //static float t = 0.0f; if (ImGui::GetTime() - t > 0.02f) { t = ImGui::GetTime(); Addzlog("Spam %f", t); }
        ...
        if (ImGui::BeginPopupContextWindow())
        {
            if (ImGui::Selectable("Clear")) Clearzlog();
            ImGui::EndPopup();
        }

        // Display every line as a separate entry so we can change their color or add custom widgets. If you only want raw text you can use ImGui::TextUnformatted(zlog.begin(), zlog.end());
    ...
    {
        //Addzlog("cursor: %d, selection: %d-%d", data->CursorPos, data->SelectionStart, data->SelectionEnd);
...
// Usage:
//  static ExampleAppzlog my_zlog;
//  my_zlog.Addzlog("Hello %d world\n", 123);
//  my_zlog.Draw("title");
struct ExampleAppzlog
{
    ImGuiTextBuffer     Buf;
...
// Demonstrate creating a simple zlog window with basic filtering.
static void ShowExampleAppzlog(bool *p_open)
{
    static ExampleAppzlog zlog;
    ...
    {
        const char *random_words[] = { "system", "info", "warning", "error", "fatal", "notice", "zlog" };
        zlog.Addzlog("[%s] Hello, time is %.1f, frame count is %d\n", random_words[rand() % IM_ARRAYSIZE(random_words)], time, ImGui::GetFrameCount());
        last_time = time;
    }

    zlog.Draw("Example: zlog", p_open);
}
...
static void ShowExampleAppLongText(bool *p_open)
{
    ...
    ImGui::Text("Buffer contents: %d lines, %d bytes", lines, zlog.size());
    if (ImGui::Button("Clear")) { zlog.clear(); lines = 0; }
    ImGui::SameLine();
    if (ImGui::Button("Add 1000 lines"))
    {
        for (int i = 0; i < 1000; i++)
            zlog.appendf("%i The quick brown fox jumps over the lazy dog\n", lines+i);
        lines += 1000;
    }
    ImGui::BeginChild("zlog");
    switch (test_type)
    {
    case 0:
        // Single call to TextUnformatted() with a big buffer
        ImGui::TextUnformatted(zlog.begin(), zlog.end());
        break;
    case 1:
        {
