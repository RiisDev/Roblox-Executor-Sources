...
DWORD ScriptContext;
DWORD ScriptContextVFTable = Rebase(0x19BF320);
...
namespace Hook {
	...
	DWORD RLSorg = 0;
	DWORD hookaddr = 0x851ce0; //gettop addr
...
int luawritefile(lua_State* rlua_State) {
	...
	char path[MAX_PATH];
	GetFile("Main.dll", "", path, MAX_PATH);
...
int __FURK(lua_State* L) {
	r_lua_getglobal(RLS, "print");
	r_lua_pushstring(RLS, "Furk Bitch");
	return true;
}
...
int WINAPI h_MessageBox(
	...
	if (str_cap == "Roblox Crash") {
		...
		str_lp = "Exploit crashed.. but don't worry! use alt accounts when you gonna use exploits";
		str_cap = "Exploit has crashed";
	}
...
int ImperiousSoRetardedHisErrorFunctionBAIT() {
	...
	ConsoleBypass("Your Exploit Name|Roblox Debug Console");
...
//	std::cout << iblue << "[OutPut] Lua Open" << endl;
	...
	int unk[] = { NULL, NULL };
	r_lua_sandboxthread(RLS, 6, (int)unk);
...
//	std::cout << ipurple << " ClipBoards" << endl;
	...
	lua_newtable(LS); //Makes _G function work :) 
	lua_setglobal(LS, "_G");
	lua_register(LS, "require", custom_require);
	lua_register(LS, "error", error);
	lua_register(LS, "__FURK", __FURK);
	...
	std::cout << igreen << "" << igreen << std::endl;
    std::cout << igreen << "" << igreen << std::endl;
	std::cout << igreen << "~~~ Developers ~~~" << igreen << std::endl;
	std::cout << ipurple << "- Athoi 21 (Fixed All problems x updated) " << ipurple << std::endl;
	std::cout << ipurple << "- Imperious (Creator/Broker) " << ipurple << std::endl;

//	std::cout << ired << "[Lua_Register]";
	KeyBoardLibs::RegisterKeyBoardLibs(LS);
	MouseLibs::RegisterMouseLibs(LS);
//	std::cout << ipurple << " Mouse And KeyPress" << endl;
//	std::cout << ired << "[Testing]";
	...
	std::cout << iyellow << "this window will close in  7 sec" << endl;
	
	luaL_dostring(LS, "game.StarterGui:SetCore('SendNotification', {Duration = 10; Title='Exploit'; Text='Successfully Injected'})");

	Sleep(7000);
	::ShowWindow(GetConsoleWindow(), SW_HIDE);
//	ShowIMGUI(LS);
	//system("start https://www.youtube.com/channel/UCCuk4UdRYukHCHuzSTKlqpw?view_as=subscriber");
	return 1;
