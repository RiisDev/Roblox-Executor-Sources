...
//#include "C:\Users\Erdit\Desktop\Stuff\vmp3\Include\C\VMProtectSDK.h" //change this to ur own one, im using the vmp3 package

/*
We will come forth, System48 will become the number one command based exploit of all time, a legend; something people will still dream about.
We hope.
victores sumus
*/
...
#define CLIENT_VERSION "3.2.7 | THE FINAL STAND"
#define DEBUG_MODE 1

#define ScanAddress(address) (address - 0x400000 + (DWORD)GetModuleHandle(0))
#define SCRIPTCONTEXT 0x100B590
...
vector<int> GetPlayers(string Name) {
	...
	for (DWORD i = 0; i < Arguments.size(); i++) {
		...
		if (Input == "me" || Input == "Me") {
			...
			vector<int> P2 = getChildren(Players);
			...
			vector<int> Plrs = getChildren(Players);
...
void ChatCommands(string Message) {
	vector<string> Arguments = SplitString(Message, ' ');
...
int InitilizeSystem48() {
	...
	DWORD* ScriptContext = (DWORD*)Memory::Scan((char*)&VFTable);
...
void Whitelist() {
	//("SysFunc: Authorisation Method");
	string hwidhere = GetMachineID();
	string url = DownloadURL("http://www.timberman.xyz/AzuLXHosting/Auth2.php?hwid=" + hwidhere);
	...
	if (((url2 - 420) / 69) == hwidhere2) {
		...
		MessageBox(NULL, "Authentication failed or connection attempt to the server was unsuccessful. Check your connection. If this error continues, alert a moderator on the discord.\n", "Authentication Failure", MB_OK);
		exit(1);
	}
	//VMProtectEnd();
}
