...
#define r_lua_isnoneornil(rL, n)	(r_lua_type(rL, (n)) <= 0)
...
static DWORD FnFire = ASLR(fireclickdetector_addr);
...
//Imm gonna add shit here
typedef void(lua_pushnil_CCV* lua_pushnilok)(DWORD);
lua_pushnilok r_lua_pushnil = (lua_pushnilok)unprotect((ASLR(lua_pushnil_addr)));

typedef void(lua_rawseti_CCV* panda_rawseti)(DWORD, DWORD, DWORD);
panda_rawseti r_lua_rawseti = (panda_rawseti)unprotect((ASLR(lua_rawseti_addr)));

typedef TValue* (index2adr_CCV* rrindex2)(int lst, int idx);
rrindex2 r_lua_index2 = (rrindex2)(ASLR(index2adr_addr));
...
TValue*& r_base(DWORD rL)
{
	return *reinterpret_cast<TValue**>(rL + ls_base);
}
...
bool isuserdata(DWORD rL, int idx)
{
	const TValue* o = r_index2adr(rL, idx);
	return (ttype(o) == RLUA_TUSERDATA || ttype(o) == RLUA_TLIGHTUSERDATA);
}
