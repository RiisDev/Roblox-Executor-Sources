...
l_noret luaL_typeerrorsL(lua_State* L, int narg, const char* tname1, const char* tname2)
{
    const char* fname = currfuncname(L);
    const TValue* obj = luaA_toobject(L, narg);
    ...
    {
        if (fname)
            luaL_error(L, "invalid argument #%d to '%s' (%s or %s expected, got %s)", narg, fname, tname1, tname2, luaT_objtypename(L, obj));
        else
            luaL_error(L, "invalid argument #%d (%s or %s expected, got %s)", narg, tname1, tname2, luaT_objtypename(L, obj));
    }
    else
    {
        if (fname)
            luaL_error(L, "missing argument #%d to '%s' (%s or %s expected)", narg, fname, tname1, tname2);
        else
            luaL_error(L, "missing argument #%d (%s or %s expected)", narg, tname1, tname2);
    }
}


void luaL_checkarguments(lua_State* rl, int nargs, const char* name)
{
    if (lua_gettop(rl) != nargs)
    {
        if (lua_gettop(rl) > 1)
        {
            luaL_error(rl, "[%s] function got %d arguments (%d expected)", name, lua_gettop(rl), nargs);
        }
        else if (lua_gettop(rl) == 0)
        {
            luaL_error(rl, "[%s] function have no arguments (%d expected)", name, nargs);
        }
        else
        {
            luaL_error(rl, "[%s] function got 1 argument (%d expected)", name, nargs);
        }
    }
}
...
static l_noret tag_errors(lua_State* L, int narg, int tag1, int tag2)
{
    luaL_typeerrorsL(L, narg, lua_typename(L, tag1), lua_typename(L, tag2));
}

void luaL_checktypes(lua_State* L, int narg, int t1, int t2)
{
    if (lua_type(L, narg) != t1 && lua_type(L, narg) != t2)
        tag_errors(L, narg, t1, t2);
}
