...
{
    size_t size = sizeof(Table) + (h->node == reinterpret_cast<LuaNode*>(seliware::address::luah_dummynode) ? 0 : sizenode(h) * sizeof(LuaNode)) + h->sizearray * sizeof(TValue);

    fprintf(f, "{\"type\":\"table\",\"cat\":%d,\"size\":%d", h->memcat, int(size));

    if (h->node != reinterpret_cast<LuaNode*>(seliware::address::luah_dummynode))
    {
        fprintf(f, ",\"pairs\":[");
