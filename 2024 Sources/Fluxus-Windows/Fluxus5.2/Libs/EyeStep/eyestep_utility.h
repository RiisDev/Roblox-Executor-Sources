...
namespace EyeStep
{
	...
	namespace util
	{
		...
		extern std::string GetCallingConvention(uint32_t func);

		extern std::string GetBCallingConvention(uint32_t func);
		...
		extern DWORD FindPattern(const char* aob, const char* mask);
		...
		extern int ArgCount(uint32_t func);
	...
	namespace scanner
	{
		...
		extern std::string pptrstring(uint32_t ptr); // converts a pointer to a scan-able AOB
	}
}
