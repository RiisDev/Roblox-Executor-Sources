...
namespace EyeStep
{
	...
	namespace util
	{
		...
		{
			return (DWORD)GetModuleHandle(0) + address;
		}

		uint32_t aslr(uint32_t address)
		{
			return ((DWORD)GetModuleHandle(0) + address) - 0x400000;
		}

		uint32_t raslr(uint32_t address)
		{
			return (address - (DWORD)GetModuleHandle(0)) + 0x400000;
		}
		...
		bool isCall(uint32_t address)
		{
			return (
				isRel(address)
				&& getRel(address) > (DWORD)GetModuleHandle(0)
				&& getRel(address) < (DWORD)GetModuleHandle(0) + base_module_size
				);
		}
		...
		std::string GetBCallingConvention(uint32_t func)
		{
			function_info info;
			info.analyze(func);
			if (info.convention == c_cdecl) {
				return "Cdecl";
			}
			else if (info.convention == c_stdcall) {
				return "Stdcall";
			}
			else if (info.convention == c_fastcall) {
				return "Fastcall";
			}
			else if (info.convention == c_thiscall) {
				return "Thiscall";
			}
		...
		std::string GetCallingConvention(uint32_t func)
		{
			function_info info;
			info.analyze(func);
	...
	namespace scanner
	{
		...
		scan_results scan(const char* aob, bool code, int align, int endresult, std::vector<scan_check>checks)
		{
			...
			if (!code)
			{
				...
				start = (DWORD)GetModuleHandle(0) + base_module_size;
				end = reinterpret_cast<uint32_t>(siSysInfo.lpMaximumApplicationAddress);
			}
			else
			{
				start = (DWORD)GetModuleHandle(0);
				end = (DWORD)GetModuleHandle(0) + base_module_size;
			}
		...
		std::string pptrstring(uint32_t ptr)
		{
			std::string aob = "";
		...
		scan_results scan_xrefs(const char* str, int nresult)
		{
			...
			{
				return scan(pptrstring(result_list.back()).c_str());
			}
			else {
		...
		scan_results scan_xrefs(uint32_t func)
		{
			...
			uint32_t start = (DWORD)GetModuleHandle(0);
			uint32_t end = (DWORD)GetModuleHandle(0) + base_module_size;
