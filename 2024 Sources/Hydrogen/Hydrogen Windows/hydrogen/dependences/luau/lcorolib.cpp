...
std::unordered_map<Closure*, lua_State*> newcclosure_map{ };

static int hyd_auxwrapy( lua_State* L )
{
    lua_State* co = nullptr;

    auto res = newcclosure_map.find( clvalue(( L->ci )->func) );

    if ( res != newcclosure_map.end( ) )
        co = res->second;
    else
        luaL_error( L, "Error during newcclosure" );
...
static int hyd_auxwrapcont( lua_State* L, int status )
{
    lua_State* co = nullptr;
...
int hyd_cowrap( lua_State* L )
{

    lua_pushcclosurek( L, hyd_auxwrapy, NULL, 0, hyd_auxwrapcont );
    ...
    newcclosure_map[ clvalue(index2addr( L, -1 )) ] = NL;
