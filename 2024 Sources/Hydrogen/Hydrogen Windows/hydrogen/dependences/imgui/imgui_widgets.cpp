...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    const auto min = bb_frame.Min + ((axis == ImGuiAxis_X) ? ImVec2{ 18, 0 } : ImVec2{ 0, 17 });
    const auto max = bb_frame.Max - ((axis == ImGuiAxis_X) ? ImVec2{ 18, 0 } : ImVec2{ 0, 17 });
    ...
    // Render
    const ImU32 bg_col = ImColor{ 46, 52, 64 };
    window->DrawList->AddRectFilled(bb_frame.Min, bb_frame.Max, bg_col, window->WindowRounding, flags);
    ImRect grab_rect;
    if (axis == ImGuiAxis_X)
        grab_rect = ImRect(ImLerp(bb.Min.x, bb.Max.x, grab_v_norm), bb.Min.y, ImLerp(bb.Min.x, bb.Max.x, grab_v_norm) + grab_h_pixels, bb.Max.y);
    ...
    const auto col1 = ImColor{ 76,86,106 };
    const auto col2 = ImColor{ 67,76,94 };

    const auto top_left = col1;
    const auto top_right = axis == ImGuiAxis_X ? col1 : col2;
    const auto bottom_left = axis == ImGuiAxis_X ? col2 : col1;
    const auto bottom_right = col2;

    window->DrawList->AddRectFilled(grab_rect.Min, grab_rect.Max, ImColor{ 46, 52, 64 }, 2);
    window->DrawList->AddRectFilledMultiColor(grab_rect.Min + ImVec2{ 1.5f, 1.5f }, grab_rect.Max - ImVec2{ 1.f, 1.f }, top_left, top_right, bottom_right, bottom_left);

    if (axis == ImGuiAxis_X)
    {
        window->DrawList->AddRectFilled(bb_frame.Min + ImVec2{ 0.0f, 0 }, bb_frame.Min + ImVec2{ 18.f, 17.f }, ImColor{ 46, 52, 64 }, 1);
        window->DrawList->AddRectFilled(bb_frame.Min + ImVec2{ 1.0f, 1.f }, bb_frame.Min + ImVec2{ 17, 16.f }, ImColor{ 67,76,94 }, 1);

        window->DrawList->AddText(nullptr, 7, bb_frame.Min + ImVec2{ 6.f, 8.f }, ImColor{ 175, 175, 175 }, (const char*)u8"\uf0d9");

        window->DrawList->AddRectFilled(bb_frame.Max - ImVec2{ 18.f, 17.f }, bb_frame.Max - ImVec2{ 0.f, 0 }, ImColor{ 46, 52, 64 }, 3);
        window->DrawList->AddRectFilled(bb_frame.Max - ImVec2{ 17, 16.f }, bb_frame.Max - ImVec2{ 1.0f, 1.f }, ImColor{ 67,76,94 }, 3);

        window->DrawList->AddText(nullptr, 7, bb_frame.Max - ImVec2{ 11.f, 9.f }, ImColor{ 175, 175, 175 }, (const char*)u8"\uf0da");
    }
    else
    {
        window->DrawList->AddRectFilled(bb_frame.Min + ImVec2{ 0.0f, 0 }, bb_frame.Min + ImVec2{ 18.f, 17.f }, ImColor{ 46, 52, 64 }, 1);
        ...
        window->DrawList->AddText(nullptr, 7, bb_frame.Min + ImVec2{ 5.f, 8.f }, ImColor{ 175, 175, 175 }, (const char*)u8"\uf0d8");

        window->DrawList->AddRectFilled(bb_frame.Max - ImVec2{ 18.f, 17.f }, bb_frame.Max - ImVec2{ 0.f, 0 }, ImColor{ 46, 52, 64 }, 3);
        window->DrawList->AddRectFilled(bb_frame.Max - ImVec2{ 17, 16.f }, bb_frame.Max - ImVec2{ 1.0f, 1.f }, ImColor{ 67,76,94 }, 3);

        window->DrawList->AddText(nullptr, 7, bb_frame.Max - ImVec2{ 13.f, 9.f }, ImColor{ 175, 175, 175 }, (const char*)u8"\uf0d7");
    }
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    BeginChildFrame(id, frame_bb.GetSize(), ImGuiWindowFlags_AlwaysVerticalScrollbar);
    return true;
}
