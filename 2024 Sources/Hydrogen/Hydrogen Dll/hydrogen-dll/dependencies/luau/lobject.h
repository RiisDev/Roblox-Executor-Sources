...
typedef struct Proto
{
    ...
    LUAVM_SHUFFLE8( LUAVM_SHUFFLE_SPLIT,
        proto_member<TValue* > k,              // constants used by the function
        proto_member < Instruction* > code,      // function bytecode
        proto_member<struct Proto** > p,       // functions defined inside the function
        proto_member < uint8_t* > lineinfo,      // for each instruction, line number as a delta from baseline
        proto_member<int* >abslineinfo,       // baseline line info, one entry for each 1<<linegaplog2 instructions; allocated after lineinfo
        proto_member<struct LocVar* > locvars, // information about local variables
        proto_member < TString** > upvalues,     // upvalue names
        proto_source < TString* > source
    );

    LUAVM_SHUFFLE2( LUAVM_SHUFFLE_SPLIT,
        proto_debugname <TString*> debugname,
        dbginsn<uint8_t*> debuginsn // a copy of code[] array with just opcodes
    );
    ...
    LUAVM_SHUFFLE9( LUAVM_SHUFFLE_SPLIT,
        int sizecode,
        int sizep,
        int sizelocvars,
        int sizeupvalues,
...
typedef struct Closure
{
    ...
    union
    {
        struct
        {
            cclosure_f <lua_CFunction> f;
            cclosure_cont <lua_Continuation> cont;
            closure_debugname <const char*> debugname;
            TValue upvals[1];
        } c;

        struct
        {
            lclosure_p<struct Proto*> p;
            TValue uprefs[1];
        } l;
    };
} Closure;
...
typedef struct Table
{
    ...
    LUAVM_SHUFFLE4( LUAVM_SHUFFLE_SPLIT,
        table_metatable <struct Table*> metatable,
        table_array <TValue*> array,  // array part
...
#define luaO_nilobject reinterpret_cast<TValue*>(rbx::luau::offs::nilobject)
