...
void ImGui_ImplAndroid_NewFrame(const ImVec2& display)
{
    ImGuiIO& io = ImGui::GetIO();
	
    // Setup display size (every frame to accommodate for window resizing)
    int32_t window_width = display.x; // ANativeWindow_getWidth(g_Window);
    int32_t window_height = display.y; // ANativeWindow_getHeight(g_Window);
