...
std::int32_t roblox::environment::debug::get_upvalue(lua_State* state)
{
	...
	const auto closure = *reinterpret_cast<Closure**>(managers::function_mgr.index2addr(state, -1));
...
std::int32_t roblox::environment::debug::set_upvalue(lua_State* state)
{
	...
	setobj(state, &closure->l.uprefs[idx - 1], managers::function_mgr.index2addr(state, 3));
...
std::int32_t roblox::environment::debug::set_constant(lua_State* state)
{
	...
	setobj(state, constant, managers::function_mgr.index2addr(state, 3));
...
std::int32_t roblox::environment::debug::get_proto(lua_State* state)
{
	...
	managers::roblox_mgr.function_map.insert({ clvalue(managers::function_mgr.index2addr(state, -1)), nullptr });
...
std::int32_t roblox::environment::debug::set_stack(lua_State* state)
{
	...
	setobj(state, (state->ci - level)->base + index, managers::function_mgr.index2addr(state, 3));
