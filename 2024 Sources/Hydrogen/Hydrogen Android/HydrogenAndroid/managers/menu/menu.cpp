...
fs::path core_path = "/data/data/com.roblox.client/cache/hydrogen/";


void managers::menu_manager_t::initiate( )
{
	static std::once_flag early_auth;
	std::call_once( early_auth, [ & ]( )
	{
		std::string base = "https://api.hydrogen.sh/gateway/auth?key=";

		std::ifstream stream( core_path / "key" );
		...
		if ( stream.is_open( ) )
		{
			...
			base += key + "&hwid=" + managers::jni_mgr.get_hwid( );

			http_t http{ managers::jni_mgr.get_jvm( ) };
			const auto [code, res] = http.get( base.c_str( ) );

			if ( code != 200 )
			{
				notifications.emplace_back( notification_type_t::error, "Saved Key has likely Expired" );
			}

			if ( res.find( "expiresAt" ) != std::string::npos )
			{
				authed = true;
			...
			menu::window( "blob", blob_pos, blob_size, [ & ]( )
			{
				...
				if ( menu_toggled )
				{
					...
					if ( menu::button_behavior( button2_pos, button2_size ) == menu::button_result_t::press )
					{
						...
						const auto key = managers::jni_mgr.get_clipboard( );
						...
						if ( code != 200 )
						{
							notifications.emplace_back( notification_type_t::error, std::string{ "Attempt: " } + base.c_str( ) );
							notifications.emplace_back( notification_type_t::error, "Failed. Check that your key is valid." );
						}

						if ( res.find( "expiresAt" ) != std::string::npos )
						{
							std::ofstream stream( core_path / "key", std::ios::beg | std::ios::binary );
...
managers::menu_manager_t::notification_t::notification_t( notification_type_t tt, std::string text )
{
	this->tt = tt;
	this->text = text;
}
