    ...
    if (!mLines.empty())
    {
        ...
        while (lineNo <= lineMax)
        {
            ...
            if (sstart != -1 && ssend != -1 && sstart < ssend)
            {
                ...
                drawList->AddRectFilled(vstart, vend, mPalette[(int)PaletteIndex::Selection], vend.y - vstart.y);
            }
            ...
            if (mState.mCursorPosition.mLine == lineNo)
            {
                ...
                if (!HasSelection())
                {
                    ...
                    auto rect_rounding = end.y - start.y;
                    drawList->AddRectFilled(start, end, mPalette[(int)(focused ? PaletteIndex::CurrentLineFill : PaletteIndex::CurrentLineFillInactive)], rect_rounding);
                    drawList->AddRect(start, end, mPalette[(int)PaletteIndex::CurrentLineEdge], rect_rounding);
                }
...
void TextEditor::Render(const char* aTitle, const ImVec2& aSize, bool aBorder)
{
    ...
    if (!mIgnoreImGuiChild)
        ImGui::BeginChild(aTitle, aSize, aBorder, ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_AlwaysVerticalScrollbar | ImGuiWindowFlags_AlwaysHorizontalScrollbar | ImGuiWindowFlags_NoMove);
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Luau()
{
    static bool inited = false;
    static LanguageDefinition langDef;
    if (!inited)
